...
//				name = result.getTestName();
	...
	//					test.info("output:"+output);
						if (result.getThrowable() != null) {
							...
							Markup m = MarkupHelper.createLabel(output, status==Status.PASS?ExtentColor.BLUE:ExtentColor.RED);
//							test.log(status, "Test "+ status.toString().toLowerCase() + "ed "+output);
							test.log(status, m);
						}
					}
				else 
