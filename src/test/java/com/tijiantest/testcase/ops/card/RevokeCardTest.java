...
public class RevokeCardTest extends OpsBase {

	@Test(description = "撤销发卡", groups = { "qa" }, dependsOnGroups = {"crm_distributeCard" }, dataProvider = "revokeCard",ignoreMissingDependencies = true)
	public void test_revokeCard(String... args) throws SqlException {
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if ((cardManageDto.getStatus() == CardStatusEnum.USABLE.getCode()
					...
					|| cardManageDto.getStatus() == CardStatusEnum.BALANCE_RECOVERED.getCode()) {
				cmt = cardManageDto;// 撤销发卡
			}
		...
		System.out.println("要撤销的卡ID:" + cmt.getId());
		...
		HttpResult result = httpclient.post(Flag.OPS, OPS_RevokeCard, json);
		System.out.println(result.getBody());
		Assert.assertEquals(result.getCode(), HttpStatus.SC_OK);

		if (checkdb) {
			Card resultCard = CardChecker.getCardById(card.getId());
			Assert.assertEquals(resultCard.getStatus(), CardStatusEnum.CANCELLED.getCode());
		}
	}

	@DataProvider(name = "revokeCard")
	public Iterator<String[]> revokeCard() {
		try {
			return CvsFileUtil.fromCvsFileToIterator("./csv/card/ops/revokeCard.csv", 10);
