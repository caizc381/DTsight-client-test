 ...
 *
 */
public class NoLoginWeixinTest extends MainBaseNoLogin{	
	protected static int offmealId = 0;
	protected static String wxIdCard = null; //新用户免登陆列表
	protected static int wxpayOrder = 0; //免登陆订单列表
	protected static String wxPassword = null; //账户密码,与上列身份证对应
	protected static String wxMobile = null; //账户手机号码
	protected static String wxOrderNum = null;//订单编号
	@Test(description = "验证是否登陆",groups = {"qa","main_validateNologinWxpay"})
	public void test_01_validateLoginAddToken(){
		checkNoLoginEnv(hc4,defSite);
	}
	...
	public void test_02_noLoginBook_wx(String ... args) throws SqlException{
		 //传参
		 ...
		 params.add(_p);
		 params.add(umobile);
		 HttpResult result = hc4.post(Flag.MAIN,Account_MobileValidationCode,params);
		...
		bookParams.setScene(5);
		 ...
		 result = hc4.post(Flag.MAIN,NoLoginBook, params,JSON.toJSONString(bookParams));
		 log.info("result...."+result.getBody());
		 Assert.assertEquals(result.getCode(),HttpStatus.SC_OK);
		 wxpayOrder = Integer.parseInt(JsonPath.read(result.getBody(), "$.orderId").toString());
		 if(checkdb){
			 //tb_user
			 ...
			 Assert.assertEquals(list.size(),1);
			 map = list.get(0);
			 Assert.assertEquals(Integer.parseInt(map.get("id").toString()),wxpayOrder);
			 ...
			 wxPassword = sms.split("初始密码：")[1].split("，")[0];
			 
			 if(checkmongo){
					waitto(mongoWaitTime);
					List<Map<String,Object>> mlist = MongoDBUtils.query("{'id':"+wxpayOrder+"}", MONGO_COLLECTION);
	...
	@Test(description = "免登微信支付，调用统一的支付方法" ,groups= {"qa"}, dependsOnMethods = "test_02_noLoginBook_wx",enabled = false)
	public void test_03_goWxPayNoLogin() throws Exception{
		DBMapper.update("update tb_hospital_settings set weixin_pay = 1 where hospital_id = "+defHospitalId);
		...
		int aliPay = Integer.parseInt(payMethods.get(HospitalParam.ALI_PAY).toString());
		 if(aliPay == 1){ 
			 ...
			 jo.put("orderId", wxpayOrder);
			 jo.put("_site", defSite);
			 jo.put("site",defSite);
			 jo.put("subSite", "/"+defSite);
			 jo.put("payType", PayMethod.Wxpay);
			 jo.put("client", "wx");
			 jo.put("p", "");
			 
			 
			 
			 HttpResult result = hc4.post(Flag.MAIN,Main_OrderPay,jo.toJSONString());
			 Assert.assertEquals(result.getCode(),HttpStatus.SC_OK);
			 String body = result.getBody();
			 log.info("微信页面返回...."+body);
			
			 Map<String,String> cos = hc4.getCookies();
			 System.out.println("openid............."+cos.get("openid"));
			...
			if(checkdb){
					...
					List<Map<String,Object>> list = DBMapper.query(sql, wxpayOrder);
					...
					wxOrderNum = orderMap.get("order_num").toString();
					Assert.assertEquals(status,OrderStatus.PAYING.intValue()); //支付中
					Assert.assertEquals(payRes.getRefOrderNum(),wxOrderNum);
					Assert.assertEquals(payRes.getRequireAmount()+"",orderPrice+"");
					//校验tb_paymentrecord
					String paymentSql = "select p.* from tb_paymentrecord p ,tb_payment_method m  where p.order_id = "+wxpayOrder + " and p.payment_method_id = m.id and m.type = "+PaymentTypeEnum.Weixin.getCode();
					...
					//校验tb_paylog
					String paylogSql = "select * from tb_paylog where order_id = "+wxpayOrder + " order by id desc limit 1";
					...
					Assert.assertEquals(Integer.parseInt(paylogMap.get("trade_body").toString()),OrderChecker.getOrderInfo(wxpayOrder).getOperatorId().intValue());
					...
					List<TradeOrder> tradeOrderList = PayChecker.getTradeOrderByOrderNum(wxOrderNum,TradeType.pay);
					...
					Assert.assertEquals(to.getPayMethodType().intValue(),PayConstants.PayMethodBit.WxpayBit);
					...
					List<TradePayRecord> tradePayRecList = PayChecker.getTradePayRecordByOrderNum(wxOrderNum, to.getTradeOrderNum(),PayConstants.OrderType.MytijianOrder);
					...
					Assert.assertEquals(tpr.getTradeMethodConfigId().intValue(),PayChecker.getSuitablePayMethodId(defHospitalId, PayConstants.PayMethodBit.WxpayBit));
					...
					Assert.assertEquals(tpr.getPayTradeAccountId().intValue(),PayChecker.getTradeAccountByOrderId(wxpayOrder));
					Assert.assertNull(tpr.getPayTradeSubaccountId());
					Assert.assertNull(tpr.getPayTradeSubaccountType());
					int dbReceiveTradeAccountId = PayChecker.getSuitableReceiveMethodId(defHospitalId, PayConstants.PayMethodBit.WxpayBit);
	...
	@AfterClass(description = "撤销免登陆微信预约的订单",alwaysRun = true)
	public void  test_revorkeOrder() throws SqlException{
			onceLoginInSystem(hc4, Flag.MAIN, wxMobile, wxPassword);
			int id =  wxpayOrder;
		waitto(1);//等待1s再撤销订单

		//撤销订单
			OrderChecker.Run_MainOrderRevokeOrder(hc4, id, false, true, true);
		    //校验tb_paymentrecord 无数据插入
			String paymentSql = "select p.* from tb_paymentrecord p ,tb_payment_method m  where p.order_id = "+wxpayOrder + " and p.trade_type = "+TradeTypeEnum.unifyRefund.getCode() + " and  p.payment_method_id = m.id and m.type = "+PaymentTypeEnum.Alipay.getCode();
			List<Map<String,Object>> paymentList = DBMapper.query(paymentSql);
			Assert.assertEquals(paymentList.size(),0);
			//校验tb_paylog 不增加数据，支付宝支付未成功，撤销不插入新数据
			String paylogSql = "select * from tb_paylog where order_id = "+wxpayOrder + " and trade_type = 1011 order by id ";
			List<Map<String,Object>> paylogList = DBMapper.query(paylogSql);
			Assert.assertEquals(paylogList.size(),0);
			//校验tb_trade_order【交易订单表】
			List<TradeOrder> tradeRefundList = PayChecker.getTradeOrderByOrderNum(wxOrderNum,TradeType.refund);
			Assert.assertEquals(tradeRefundList.size(),0); //退款无记录
			HttpResult result = hc4.post(Flag.MAIN,MainDeleteOrder, id);
