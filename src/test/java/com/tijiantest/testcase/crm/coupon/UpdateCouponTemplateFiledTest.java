...
/**
 * 编辑优惠券模板
 * 位置：CRM->维护后台->营销->优惠券模板
 * 操作：编辑
 *
 * @author huifang
 */
public class UpdateCouponTemplateFiledTest extends CrmBase {

    @Test(description = "发放优惠券之前，修改优惠券模板内容",groups = {"qa"},dataProvider = "updateCouponTemplateFiled_beforeSend")
    public void test_01_updateCouponTemplateFiled(String ...args) throws SqlException, ParseException {
        //STEP1:数据文件分割入参
        String name = args[2];
...
//        String sourceStr = args[10];
        ...
        if(batchNum == null){
            log.error("医院"+defhospital.getName()+"中没有新建的优惠券模板，不适用于本用例，本用例无法执行");
            return;
        }
        //STEP3:入参转换对象
        ...
        //STEP5.调用接口
        HttpResult result = httpclient.post(Coupon_UpdateCouponTemplateFiled, JSON.toJSONString(ct));
    ...
    public void test_02_updateCouponTemplateFiled(String ...args) throws SqlException, ParseException {
        //STEP1:数据文件分割入参
        String name = args[2];
        String quantityStr = args[3];
...
//        String sourceStr = args[5];
        ...
        if(checkdb){
            //随机从数据库查询1条新建的优惠券模板
            String sql = "select c.* from tb_coupon_template c  where c.status = 2 and c.end_time > now() and is_deleted = 0 and organization_id = "+defhospital.getId();
            List<CouponTemplate> couponTemplateList = PayChecker.getCouponTemplateList(sql);
            if(couponTemplateList != null && couponTemplateList.size() > 0){
                batchNum = couponTemplateList.get(0).getBatchNum();
                beforeQuatity = couponTemplateList.get(0).getQuantity();
        ...
        if(batchNum == null){
            log.error("医院"+defhospital.getName()+"中没有已发放的优惠券模板，不适用于本用例，本用例无法执行");
            return;
        }
        //STEP3:入参转换对象
        ...
        if(checkdb){
            ...
            Assert.assertEquals(afterCreditAccount.getBalance(),beforeBalance);
            Assert.assertEquals(afterCreditAccount.getCreditLimit(),beforeCreditLimit);
            int calculate_feezeCreditLimit = beforeCreditFreezeLimit + quantity * beforePrice -  beforeQuatity * beforePrice;
            Assert.assertEquals(afterCreditAccount.getFreezeCreditLimit(),calculate_feezeCreditLimit);//现在的冻结金额 = 原来的冻结金额+之前发放优惠券的面值*数量 - 现在发放优惠券的面值*数量
    ...
    public Iterator<String[]> updateCouponTemplateFiled_beforeSend(){
        try {
            return CvsFileUtil.fromCvsFileToIterator("./csv/coupon/crm/updateCouponTemplateFiled_beforeSend.csv",18);
    ...
    public Iterator<String[]> updateCouponTemplateFiled_afterSend(){
        try {
            return CvsFileUtil.fromCvsFileToIterator("./csv/coupon/crm/updateCouponTemplateFiled_afterSend.csv",18);
        } catch (FileNotFoundException e) {
            // TODO 自动生成的 catch 块
            e.printStackTrace();
        } catch (UnsupportedEncodingException e) {
