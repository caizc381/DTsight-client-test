 ...
 * @author huifang
 */
public class FindCouponTest extends CrmBase {
    ...
    public void test_01_findCoupon(String ...args) throws SqlException, ParseException {
        ...
        if(checkdb){
            ...
            String sql = "select c.* from tb_coupon_template c, tb_coupon_template_statistics s  where c.batch_num = s.template_batch_num and  c.status =2  and s.received_num >0 and c.is_deleted = 0 and c.organization_id = "+ organizationId ;
        ...
        if(batchNum == null){
            log.error("医院 "+organizationId+"中没有已经领取过且还在发放状态的优惠券版本，本用例无法执行");
            return;
        }
        ...
        //STEP4:调用接口
        HttpResult result = httpclient.post(Coupon_FindCoupon, JSON.toJSONString(vo));
        //STEP5:验证返回值&DB
...
//            sql += " and organization_id = "+organizationId;//机构ID
    ...
    public Iterator<String[]> findCoupon(){
        try {
            return CvsFileUtil.fromCvsFileToIterator("./csv/coupon/crm/findCoupon.csv",18);
