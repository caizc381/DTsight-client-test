...
public class CvsFileUtil {
    ...
    /**
     * 
     * @param filename cvs文件名称
     * @param columnsize 一行多少列
     * @return
     * @throws FileNotFoundException
     * @throws UnsupportedEncodingException
     * 去除第一行描述头
     */
	public static Iterator<String[]> fromCvsFileToIterator(String filename,int columnsize) throws FileNotFoundException, UnsupportedEncodingException{
		 ArrayList<String[]> list = new ArrayList<String[]>();
		 fis = new FileInputStream(filename);  
	     isw = new InputStreamReader(fis, ENCODE);  
	     br = new BufferedReader(isw);  
		 try {
			String cvsline = readLine(); //去除第一行描述头
			while((cvsline = readLine())!=null){
			    if(cvsline.trim().equals(""))
			        continue;
				list.add(fromCSVLine(cvsline,columnsize));//case列表
			};
		} catch (Exception e) {
			// TODO 自动生成的 catch 块
			e.printStackTrace();
	...
	public static String[][] fromCvsFileToArrays(String filename,int columnsize) throws FileNotFoundException, UnsupportedEncodingException{
		 ArrayList<String[]> list = new ArrayList<String[]>();
		 fis = new FileInputStream(filename);  
	     isw = new InputStreamReader(fis, ENCODE);  
	     br = new BufferedReader(isw);  
		 ...
		 String[][] arrays = new String[list.size()][columnsize];
     ...
     *
     */  
    public static String[] fromCSVLine(String linestrs, int size) {  
        ArrayList<String> tmpArray = fromCSVLinetoArray(linestrs);  
     ...
     */  
    public static ArrayList<String> fromCSVLinetoArray(String linestrs) {  
        if (linestrs == null || linestrs.length() == 0) {  
            return new ArrayList<String>();  
        }  
        int currentPosition = 0;  
        int maxPosition = linestrs.length();  
        int nextComma = 0;  
        ArrayList<String> rtnArray = new ArrayList<String>();  
        while (currentPosition < maxPosition) {  
            nextComma = nextComma(linestrs, currentPosition);  
            rtnArray.add(nextToken(linestrs, currentPosition, nextComma));  
